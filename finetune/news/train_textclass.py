...
# We use and adapt this NEWS script, originally from the MasakhaNEWS repository (https://github.com/masakhane-io/masakhane-news)
# We adapt this script to use custom configuration files for our ELC-BERT model
# coding=utf-8
...
from transformers import AutoModelForSequenceClassification, AutoConfig, AutoTokenizer, AutoModel, PreTrainedTokenizerFast
from tqdm import tqdm, trange
from util_textclass import get_labels, read_instances_from_file, convert_instances_to_features_and_labels
from modeling_ltgbert import LtgBertForSequenceClassification
...
MODEL_CLASSES = {
    ...
    "ltgbert": (LtgBertConfig, LtgBertForSequenceClassification, PreTrainedTokenizerFast)
}
...
def train(args, train_dataset, dev_dataset, labels, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.model_type not in ["distilbert", "xlmroberta","ltgbert"]:
...
def evaluate(args, model, tokenizer, labels, mode, prefix="", display_res=False):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3]}
            if args.model_type not in ["distilbert","ltgbert"]:
                inputs["token_type_ids"] = (
                    batch[2] if args.model_type in ["bert"] else None
                )  # XLM and DistilBERT don't use segment_ids
            outputs = model(**inputs)
...
def main():
    ...
    config_class, model_class, tokenizer_class = LtgBertConfig,LtgBertForSequenceClassification,PreTrainedTokenizerFast
    script_dir = os.path.dirname(os.path.abspath(__file__))
    model_name_or_path = os.path.join(script_dir, args.model_name_or_path)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            checkpointConfig= config_class.from_pretrained(checkpoint)
            model = model_class.from_pretrained(checkpoint, config=checkpointConfig)
            model.to(args.device)
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        tokenizer = tokenizer_class.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case)
        outputConfig = config_class.from_pretrained(args.output_dir)
        model = model_class.from_pretrained(args.output_dir, config = outputConfig)
        model.to(args.device)
