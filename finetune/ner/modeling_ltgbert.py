# We use these configuration files from the ELC-BERT repo on huggingface (https://huggingface.co/lgcharpe/ELC_BERT_small_baby_10M/tree/main)
# in order for the model to be compatible with hugging face library
# 
# #coding=utf-8
...
class Classifier(nn.Module):
    def __init__(self, config, num_labels: int):
        super().__init__()
        # Debug: Print values before setting drop_out
        print(f"classifier_dropout: {getattr(config, 'classifier_dropout', 'Not found')}")
        print(f"hidden_dropout_prob: {getattr(config, 'hidden_dropout_prob', 'Not found')}")
    ...
    def initialize(self, hidden_size):
        std = math.sqrt(2.0 / (5.0 * hidden_size))
        print(std)
        self.nonlinearity[1].weight.data.normal_(mean=0.0, std=std)
        self.nonlinearity[-1].weight.data.normal_(mean=0.0, std=std)

        

        print("trunc",self.nonlinearity[1].weight)
        print("trunc",self.nonlinearity[1].weight)
        self.nonlinearity[1].bias.data.zero_()
        self.nonlinearity[-1].bias.data.zero_()
...
class LtgBertForTokenClassification(LtgBertModel):
    ...
    def __init__(self, config):
        ...
        self.num_labels = config.num_labels
        self.head = Classifier(config, self.num_labels)
        print(f"Initialized LtgBertForTokenClassification with {self.num_labels} labels")
    ...
    def forward(
        ...
        print(f"Forward pass started. Input shape: {input_ids.shape}")
        
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict

        sequence_output, contextualized_embeddings, attention_probs = self.get_contextualized_embeddings(input_ids, attention_mask)
        print(f"Sequence output shape: {sequence_output.shape}")
        print(f"Sequence output mean: {sequence_output.mean().item():.6f}, std: {sequence_output.std().item():.6f}")
        ...
        # Debug classification layer weights and biases
        print("Classification layer weights stats:")
        print(f"Mean: {self.head.nonlinearity[-1].weight.mean().item():.6f}")
        print(f"Std: {self.head.nonlinearity[-1].weight.std().item():.6f}")
        print(f"Min: {self.head.nonlinearity[-1].weight.min().item():.6f}")
        print(f"Max: {self.head.nonlinearity[-1].weight.max().item():.6f}")
        
        print("Classification layer biases:")
        print(self.head.nonlinearity[-1].bias)
